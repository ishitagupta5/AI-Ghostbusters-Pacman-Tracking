...
def read_command(argv):
    ...
    parser.set_defaults(generate_public_tests=False)
...
def evaluate(generate_solutions, test_root, module_dict, exceptionMap=ERROR_HINT_MAP,
...
def copy(src_dir, dest_dir, filename):
    ...
    print("Copying %s -> %s" % (src_filename, dest_filename))
    shutil.copy(src_filename, dest_filename)
    # with open(os.path.join(src_dir, filename), 'r') as f1:
    #     with open(os.path.join(dest_dir, filename), 'w') as f2:
    #         f2.write(f1.read())

def generate_public_tests(module_dict, private_root='private_test_cases', public_root='test_cases'):
    for module in module_dict:
        setattr(sys.modules[__name__], module, module_dict[module])

    if not os.path.exists(public_root): os.mkdir(public_root)
    copy(private_root, public_root, 'CONFIG')
    for q in sorted(os.listdir(private_root)):
        private_subdir_path = os.path.join(private_root, q)
        public_subdir_path = os.path.join(public_root, q)
        ...
        # create a question object
        question_dict = test_parser.TestParser(os.path.join(public_subdir_path, 'CONFIG')).parse()
        question_class = getattr(test_classes, question_dict['class'])
        question = question_class(question_dict, get_display(False))
        ...
        for t in sorted(tests):
            ...
            test_case.create_public_version()
            test_case.emit_public_version(public_test_file)
# END SOLUTION NO PROMPT
...
if __name__ == '__main__':
    ...
    # BEGIN SOLUTION NO PROMPT
    if options.generate_public_tests:
        generate_public_tests(module_dict)
        sys.exit()
    # END SOLUTION NO PROMPT
